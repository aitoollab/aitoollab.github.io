...
FOOTER_HTML = """    <section class="conversion-bridge" data-conversion-bridge style="margin:40px 0;padding:24px;border:1px solid var(--border,#30363d);border-radius:12px;background:rgba(37,99,235,.08);text-align:center;">
        <strong>想了解真实可参与的AI项目？</strong>
        <p>这篇内容先解决一个具体问题。想继续了解公众号贴图、内容变现和带货项目，可以先看项目说明与案例，再添加个人微信咨询。</p>
        <div style="display:flex;gap:10px;flex-wrap:wrap;justify-content:center;">
            <a href="/projects/" style="padding:8px 14px;border:1px solid var(--border,#30363d);border-radius:7px;">查看当前项目</a>
            <a href="/projects/wechat-sticker/" style="padding:8px 14px;background:#2563eb;color:#fff;border-radius:7px;">领取公众号贴图项目说明</a>
        </div>
        <p style="margin-top:10px;font-size:12px;color:var(--muted,#8b949e);">加微信「一微尘」回复「贴图」可领项目说明与案例；如果你已经试过这篇方法，欢迎把结果或卡点发到 contact@aitoollab.top。</p>
    </section>
"""
...
def process_article(path):
    ...
    if 'data-conversion-bridge' in content:
        normalized = re.sub(
            r'<section class="conversion-bridge" data-conversion-bridge(?: style="[^"]*")?>.*?</section>',
            FOOTER_HTML.strip(),
